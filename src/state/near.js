...
import { env } from '../utils/api-utils';

export const { GAS, networkId, nodeUrl, walletUrl, nameSuffix, contractId } = getConfig(env);
  ...
  () =>
  	async ({ update, getState, dispatch }) => {
  		...
  		wallet.signIn = () => {
  			...
  			wallet.requestSignIn(contractId, 'Spearmint NFT API', window.location.href);
  		};
